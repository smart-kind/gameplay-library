...
| 操作 | 区域 | 反馈 |
|------|------|------|
| **点击空塔位** | 路径旁的虚线方框 | 弹出"建造菜单"（半透明覆盖层），显示3种塔的图标+攻击/射程/成本对比 |
...
**建造菜单（弹出覆盖层）**：
| 塔类型 | 图标 | 初始攻击 | 初始射程 | 建造费用 | 特点 |
|--------|------|---------|---------|---------|------|
| 箭塔 | 🏹 | 15 | 2格 | 免费 | 攻速快，单体 |
| 法塔 | 🔮 | 40 | 2.5格 | 100金币 | 减速20%，单体 |
| 炮塔 | 💣 | 100 | 3格 | 300金币 | 范围溅射(1.5格半径) |
| 电塔 | ⚡ | 5 | 1.5格 | 500金币 | 连锁闪电(3目标) |
...
| 法塔 | 40 | 0.8/s | 2.5格 | 攻击减速20%（持续2秒） | 100金币 | 攻击×1.2/级，费用×1.5 |
| 炮塔 | 100 | 0.4/s | 3格 | 范围溅射（半径1.5格，对主目标100%伤害，周围50%） | 300金币 | 攻击×1.2/级，费用×1.5 |
| 电塔 | 5 | 3.0/s | 1.5格 | 连锁闪电（最多3目标，每个造成100%伤害，不递减） | 500金币 | 攻击×1.2/级，费用×1.5 |
...
Step 4: 弹道命中敌人→敌人血量减少→显示飘字"-XX"
Step 5: 敌人血量≤0→播放消失动画→掉落金币→金币自动飞入顶部金币栏
Step 6: 敌人到达屏幕底部红线→扣1心（❤️→💔）→敌人消失
Step 7: 本波所有敌人被消灭或到达底线→进入"波次间隙"
```

**波次间隙**（操作时间，无敌人）：
```
Step 8: 底部中央显示"▶ 开始第X波"按钮（闪烁提示）
Step 9: 玩家点击空塔位→弹出建造菜单→选择塔类型→扣除金币→塔出现在位置
Step 10: 玩家点击已有塔→弹出升级对话框→点击升级→扣除金币→攻击力+20%
Step 11: 玩家点击右下角技能图标→释放技能（进入冷却，图标变暗+倒计时数字）
Step 12: 玩家点击"▶ 开始第X波"→进入下一波战斗
```
